# Item used in faction action inventory to create a new faction.
faction_action_create_new_faction_item:
    type: item
    material: player_head
    display name: Create a New Faction!
    lore:
        - <&color[#1569EA]>Click to create your own faction!
        - <&color[#1569EA]>This will automatically claim land
        - <&color[#1569EA]>in the chunk you are in.
    mechanisms:
        skull_skin: b4f61444-f5fa-4668-91f0-9e00408d6298|eyJ0ZXh0dXJlcyI6eyJTS0lOIjp7InVybCI6Imh0dHA6Ly90ZXh0dXJlcy5taW5lY3JhZnQubmV0L3RleHR1cmUvM2VkZDIwYmU5MzUyMDk0OWU2Y2U3ODlkYzRmNDNlZmFlYjI4YzcxN2VlNmJmY2JiZTAyNzgwMTQyZjcxNiJ9fX0=
# Item used in faction action inventory to join a new faction.
faction_action_join_new_faction_item:
    type: item
    material: player_head
    display name: Join a Faction!
    lore:
        - <&color[#1569EA]>Join a faction
        - <&color[#1569EA]>by being invited
        - <&color[#1569EA]>by a faction member!
    mechanisms:
        skull_skin: 0ef1f96b-9230-4c01-8d2d-1e6605d7251a|eyJ0ZXh0dXJlcyI6eyJTS0lOIjp7InVybCI6Imh0dHA6Ly90ZXh0dXJlcy5taW5lY3JhZnQubmV0L3RleHR1cmUvNWYwMGY1N2RkNjlkODFjZWEwNzZkZDFjZDJkNzAxNjlhZTkxY2JjYjQ2YzBmMWM3MTJiZjBiZGU4YmY1NDE4MSJ9fX0=

# Item used in faction action inventory to indicate that the player is not in a faction.
faction_action_not_in_any_faction_item:
    type: item
    material: player_head
    display name: You aren't in a faction.
    lore:
        - <&color[#1569EA]>Join a faction
        - <&color[#1569EA]>or create your own!
    mechanisms:
        skull_skin: d6dee635-8b08-465e-b616-bbc34fda41ad|eyJ0ZXh0dXJlcyI6eyJTS0lOIjp7InVybCI6Imh0dHA6Ly90ZXh0dXJlcy5taW5lY3JhZnQubmV0L3RleHR1cmUvMmUzZjUwYmE2MmNiZGEzZWNmNTQ3OWI2MmZlZGViZDYxZDc2NTg5NzcxY2MxOTI4NmJmMjc0NWNkNzFlNDdjNiJ9fX0=

# Item used in faction action inventory to display the name of a faction..
faction_action_faction_name_item:
    type: item
    material: player_head
    display name: FACTION NAME
    mechanisms:
        skull_skin: 06ca0142-cdb9-4a4d-9f3d-1fb220dd2003|eyJ0ZXh0dXJlcyI6eyJTS0lOIjp7InVybCI6Imh0dHA6Ly90ZXh0dXJlcy5taW5lY3JhZnQubmV0L3RleHR1cmUvNmZiMjkwYTEzZGY4ODI2N2VhNWY1ZmNmNzk2YjYxNTdmZjY0Y2NlZTVjZDM5ZDQ2OTcyNDU5MWJhYmVlZDFmNiJ9fX0=

# Item used in faction action inventory to invite players to a faction.
faction_action_invite_players_item:
    type: item
    material: player_head
    display name: Invite Members!
    mechanisms:
        skull_skin: 0beedb99-f64e-4a26-93f3-8f3af84050aa|eyJ0ZXh0dXJlcyI6eyJTS0lOIjp7InVybCI6Imh0dHA6Ly90ZXh0dXJlcy5taW5lY3JhZnQubmV0L3RleHR1cmUvN2ZmYWNjZjE3ODc5YjE3ODkxZmM1ZWY2NjQ3MmNjMDY2YTg1YmZhMzFiNmQ3ODZjMzJhZmVlNDc5NjA2OGQifX19

# Item used in faction action inventory to claim more land for a faction.
faction_action_claim_more_land_item:
    type: item
    material: player_head
    display name: Claim More Land for Your Faction!
    mechanisms:
        skull_skin: 15bec315-811e-433e-a685-bd1c622604f4|eyJ0ZXh0dXJlcyI6eyJTS0lOIjp7InVybCI6Imh0dHA6Ly90ZXh0dXJlcy5taW5lY3JhZnQubmV0L3RleHR1cmUvNDUyOGVkNDU4MDI0MDBmNDY1YjVjNGUzYTZiN2E5ZjJiNmE1YjNkNDc4YjZmZDg0OTI1Y2M1ZDk4ODM5MWM3ZCJ9fX0=
